...
// Dynamic navigation items that will be translated
function getNavItems(t: (key: string) => string) {
  return [
    { href: "/dashboard", icon: "📊", label: "Dashboard" },
    { href: "/projects", icon: "📁", label: t("nav.projects") },
    { href: "/patterns", icon: "✂️", label: t("nav.patterns") },
    { href: "/templates", icon: "📐", label: t("nav.templates") },
        ...
        <div className={styles.sidebarFooter}>
          {sidebarOpen && (
            <div className={styles.creditBadge}>
              <span>🪙</span>
              <span>{user?.credits || 0} {t("dashboard.credits")}</span>
            </div>
          )}
      ...
      <main className={styles.mainContent}>
        <header className={styles.topBar}>
          <div className={styles.searchBar}>
            ...
            <input type="text" placeholder={t("nav.search")} className={styles.searchInput} />
          </div>
